...
def preprocess(datapath, method, spacing):
    ...
    df_file = os.path.join(datapath, "rate_modelling", "processed", spacing, datafile)
    os.makedirs(
        os.path.join(datapath, "rate_modelling", "processed", spacing), exist_ok=True
    )

    if spacing == "both":
        df1 = pd.read_csv(
            os.path.join(datapath, "rate_modelling", "raw", "logspaced", datafile)
        )
        df2 = pd.read_csv(
            os.path.join(datapath, "rate_modelling", "raw", "linspaced", datafile)
        )
        df = pd.concat([df1, df2], ignore_index=True)
    else:
        df = pd.read_csv(
            os.path.join(datapath, "rate_modelling", "raw", spacing, datafile)
        )
    # n_samples = 100000
    ...
    for index, row in df_mid.iterrows():
        ...
        idx_subset = df.loc[((df["P"] == P) & (df["K"] == K) & (df["T"] == T)), :]
        subset = df.loc[idx_subset.index.values].sort_values(by="t", ascending=True)
        ...
        df_mid.loc[index, "pc_err"] = (
            np.abs((gam - subset["gamma"].values) / gam).max() * 100
        )
    ...
    match method:
        ...
        case 1:
            idx_bad_pkT = np.where(
                (df_mid["tot_change"] > 5) | (df_mid["pc_err"] > 105)
            )[0]
        case 2:
            # th_pc = 1
            # th_chg = 150
    ...
    df_bad_pkT = (
        df_mid.iloc[idx_bad_pkT].copy().sort_values(by="tot_change", ascending=True)
    )
    ...
    df_good_pc_chg = df_good_pkT.sort_values(by="pc_err", ascending=True)[
        -n_last:
    ].copy()
    df_good_t_chg = df_good_pkT.sort_values(by="tot_change", ascending=True)[
        -n_last:
    ].copy()
    ...
    for index in range(len(df_bad_pkT)):
        ...
        idx_1 = df.loc[
            ((df["P"] == P) & (df["K"] == K) & (df["T"] == T) & (df["t"] < t_mid)), :
        ]
...
def load_df(datapath, proj_dir, method, spacing="logspaced"):
    ...
    match proj_dir:
        case "rate_modelling":
            df_file = os.path.join(datapath, proj_dir, "processed", spacing, datafile)
        case "rate_integrating":
            df_file = os.path.join(
                "Results", "rate_integrating", "int_results", "combined", datafile
            )
